...
import {
  ...
  MONO_STACK,
  SANS_STACK,
} from "../../packages/renderer/src/text.js";
import {
  escapeXml,
  lines,
  tag,
  textNode,
  wrap,
} from "../../packages/renderer/src/svg/primitives.js";
...
const emojiSlack = (text: string, size: number): number => {
  ...
  for (const character of text)
    if ((character.codePointAt(0) ?? 0) > 0xffff) astral += 1;
...
const paintRuns = (
  ...
  for (const run of runs) {
    const width =
      measure(run.text, run.face, run.size) + emojiSlack(run.text, run.size);
...
const runWidth = (runs: readonly Run[]): number =>
  runs.reduce(
    (total, run) =>
      total +
      measure(run.text, run.face, run.size) +
      emojiSlack(run.text, run.size) +
      (run.chip === true ? 12 : 0),
    0,
  );
...
export const frameAsComment = (input: FrameInput): string => {
  const { doc, sections, theme, growth = false } = input;
  const palette = framePalette(theme);
  const left = CONTENT_X;
  const parts: string[] = [];
  ...
  parts.push(
    paintRuns(
      left,
      y,
      [{ text: "View", face: "sans-bold", size: 14, fill: palette.foreground }],
      palette,
    ),
  );
  gap(8);
  ...
  if (growth) {
    ...
    parts.push(
      inlineText(
        ...
        y,
        THANKS_ASK,
        { size: 13, fill: palette.foreground },
        palette,
      ),
    );
    ...
    // composer's sentence.
    {
      ...
      parts.push(
        wrap(
          "g",
          {
            transform: `translate(${round2(textX)}, ${round2(y - 12.5)})`,
      ...
      for (const line of wrapPlainText(
        ...
        parts.push(
          paintRuns(
            textX,
            y,
            [{ text: line, face: "sans", size: 13, fill: palette.foreground }],
            palette,
          ),
        );
      }
  ...
  const avatar = markAvatar(MARGIN, MARGIN);
...
/**
 * The header row GitHub actually draws over a bot comment: the account name in
 * bold, one bordered `Bot` pill, then the timestamp sentence. GitHub adds no
 * second badge — an App's comments are signed by its bot account and marked
 * only that once — so the row stops there.
 */
const APP_LOGIN = "coldtea-pr-lens";
const COMMENTED = "commented 13 minutes ago";

const headerRow = (palette: FramePalette): string => {
  const baseline = MARGIN + 23;
  const name: Run[] = [
    { text: APP_LOGIN, face: "sans-bold", size: 13, fill: palette.foreground },
  ];
  const badgeX = CONTENT_X + runWidth(name) + 8;
  const tailX = badgeX + badgeWidth("Bot") + 8;
  return lines([
    paintRuns(CONTENT_X, baseline, name, palette),
    badge(badgeX, MARGIN + 9, "Bot", palette),
    paintRuns(
      tailX,
      baseline,
      [{ text: COMMENTED, face: "sans", size: 13, fill: palette.muted }],
      palette,
    ),
  ]);
};
...
/**
 * The app's real avatar, not a stand-in: the brand mark from docs/brand, on the
 * dark tile GitHub serves it as. An avatar is one uploaded image, so it does
 * not follow the reader's theme — the dark half of the pair is drawn under both
 * palettes, which is what a maintainer sees on either GitHub theme.
 */
const markAvatar = (x: number, y: number): string =>
  lines([
    tag("rect", {
      x,
      y,
    ...
    wrap(
      ...
      lines([
        ...
        tag("circle", { cx: 152, cy: 262, r: 22, fill: "#e6edf3" }),
        tag("rect", {
          x: 222,
          y: 134,
          width: 88,
