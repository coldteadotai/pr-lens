...
| `provider` | `gemini` | or `openai-compatible`, for anything speaking `/chat/completions` |
| `model` | — | required for `openai-compatible`: the endpoint decides which names exist |
| `base-url` | — | provider endpoint, for a compatible or self-hosted server |
...
Commit `.github/pr-lens.yml` and the action picks it up — renames, exclusions, lane pins, groupings, applied over every fresh analysis. See the [schema README](../schema/README.md#repository-config) for the format, and the [agent skill](../agent-skill) if you would rather tell your coding agent to fix the map than write YAML yourself.
